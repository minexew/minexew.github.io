...
For now, the next step will be to parse and disassemble the entire .text section in a way that allows us to reassemble a bit-exact binary. There doesn't seem to be any readily available tool to do this. Objdump output could probably be regexed, but that's a crappy solution. Radare2 hangs when asked to do "pD" on the entire .text, and RetDec crashes with std::bad_alloc. What about [objconv](https://www.agner.org/optimize/objconv-instructions.pdf)?
